...
tool.Unequipped:Connect(clearHighlight)
	...
	-- Build Key Tracker Tool inside ServerStorage
	local oldKeyTool = ServerStorage:FindFirstChild("KeyTracker")
	if oldKeyTool then oldKeyTool:Destroy() end

	local keyTool = Instance.new("Tool")
	keyTool.Name = "KeyTracker"
	...
	local keyScreen = mp({
		...
		CFrame = CFrame.new(0, 0.2, -0.21),
		Material = Enum.Material.Glass,
		Color = Color3.fromRGB(30, 30, 10),
		Parent = keyTool
	})

	local keyWeld = Instance.new("WeldConstraint")
	keyWeld.Part0 = keyHandle
	keyWeld.Part1 = keyScreen
	keyWeld.Parent = keyHandle

	local keyClientScript = Instance.new("LocalScript")
	keyClientScript.Name = "RadarScript"
	keyClientScript.Source = [[
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local TweenService = game:GetService("TweenService")
...
tool.Unequipped:Connect(clearHighlight)
	...
	keyClientScript.Parent = keyTool
	keyTool.Parent = ServerStorage
end
